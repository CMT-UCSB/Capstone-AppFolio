class OpenEndedResponseChartBuilder
    def initialize(question:, elapsed:)
        @question = question
        @elapsed = elapsed
    end
    ...
    def pie_chart_data
        if !@question.nil?
            ...
            pie_chart_data = default_data.merge(OpenEndedResponse.where(question_id: @question.id, elapsed: @elapsed).group(:sentiment).count)
